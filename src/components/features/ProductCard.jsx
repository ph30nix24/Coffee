...
const ProductCard = ({item, current}) => {
  ...
  return (
    <div className='w-1/4 max-w-[230px] shadow-2xl rounded-2xl overflow-hidden items'>
        <div className='w-full h-[240px] flex justify-center items-center bg-gray-200'>
          <img className=' object-center object-contain size-3/5 hover:translate-y-2 smooth-transition' src={item.pic} alt="" />
        </div>
        <div className='p-5 relative'>
          <p className='font-semibold text-xl font-heading pb-2'>$ {item.price}</p>
          <h1 className='font-semibold text-lg font-heading capitalize'>{item.itemName}</h1>
